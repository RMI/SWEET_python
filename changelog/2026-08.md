...
- **A supplied waste depth now selects the specific IPCC category, including
  "unmanaged shallow".** The depth rule was one-directional — it raised MCF to
  0.8 above 5 m and otherwise fell through to the type default — so once the type
  default became 0.6 there was no way to reach 0.4 even for a site known to be
  shallow. A dump with a depth at or below 5 m now reads as 0.4, above 5 m as
  0.8, and only an absent depth (`None`/`NaN`) keeps the uncategorised 0.6. This
  is reachable today on the two Climate TRACE site paths
  (`City.site_only_estimate_trace`, `City.citysite_estimate_trace`, which read a
  real `waste_depth`) and on `advanced_dst`, whose `depth` field is optional and
  defaults to `None`.
  It is deliberately **not** applied on the `/sdst` request paths
  (`City.sdst_v1_5`, `City.implement_dst_changes_advanced`,
  `City.advanced_baseline`), which pass the new `trust_shallow_depth=False`. Those
  callers always send a number for depth — the DST form defaults it to 3 m and
  `sdst_prepopulate` fabricates 3 m for any dumpsite — so "no answer" cannot be
  told apart from "3 m" there, and a shallow reading would drop every default
  scenario to 0.4 rather than only the genuinely shallow sites. Closing that gap
  needs the caller to be able to send a null depth.
  ([#49](https://github.com/RMI/SWEET_python/pull/49))
