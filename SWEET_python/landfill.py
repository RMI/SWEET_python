...
class Landfill:
    def __init__(
        ...
        if self.doing_fancy_ox:
            ...
            site = Site(
                lat=self.latlon[0], lon=self.latlon[1]
            )  # Make sure this handles negative longitudes correctly
            ...
            for i, c in enumerate(cover_types):
                if c == "none":
                    ...
                    self.oxidation_potentials.append(
                        cover.calculate_oxidation_rate()
                        * areas[i]
                        * cover_thicknesses[i]
                        * 86400
                        * 365.25
                        / 1e6
                    )  # g/m³/sec * m² * m * sec/day * day/year /1e6 = tons/year

        if self.gas_capture_efficiency is None:
            self.gas_capture_efficiency = defaults_2019.gas_capture_efficiency[
                site_type
            ]
