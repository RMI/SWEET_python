...
class CityParameters(BaseModel):
    ...
    def _singapore_k(
        ...
        Lab of Professor Xunchang Fei, Nanyang Technological University College of Engineering, Singapore.
        ...
        if advanced_dst:
            ...
            nb["baseline"] = (
                self.waste_fractions["baseline"].metal
                + self.waste_fractions["baseline"].glass
                + self.waste_fractions["baseline"].plastic
                + self.waste_fractions["baseline"].other
                + self.waste_fractions["baseline"].rubber
            )
            bs["baseline"] = (
                self.waste_fractions["baseline"].wood
                + self.waste_fractions["baseline"].paper_cardboard
                + self.waste_fractions["baseline"].textiles
            )
            bf["baseline"] = (
                self.waste_fractions["baseline"].food
                + self.waste_fractions["baseline"].green
            )

            nb["scenario"] = (
                self.waste_fractions["scenario"].metal
                + self.waste_fractions["scenario"].glass
                + self.waste_fractions["scenario"].plastic
                + self.waste_fractions["scenario"].other
                + self.waste_fractions["scenario"].rubber
            )
            bs["scenario"] = (
                self.waste_fractions["scenario"].wood
                + self.waste_fractions["scenario"].paper_cardboard
                + self.waste_fractions["scenario"].textiles
            )
            bf["scenario"] = (
                self.waste_fractions["scenario"].food
                + self.waste_fractions["scenario"].green
            )
        ...
        t = self.temperature + 10  # landfill is warmer than ambient
        ...
        def create_series(
            kc, tf, fm, implement_year=None, advanced_baseline=False, advanced_dst=False
        ):
...
class City:
    def __init__(self, city_name: str):
        ...
        self.latitude = None
        self.longitude = None
        self.years_range = range(1960, 2074)
    ...
    def finish_sites_prep(self):
        ...
        for i, landfill in enumerate(landfills):
            new_landfill = Landfill(
                ...
                mcf=pd.Series(mcfs[i], index=range(lifespans[i][0], 2074)),
                ...
                gas_capture_efficiency=pd.Series(
                    gas_capture_efficiencies[i], index=range(lifespans[i][0], 2074)
                ),
                ...
                oxidation_factor=pd.Series(
                    oxidation_values[i], index=range(lifespans[i][0], 2074)
                ),
                rmi_id=rmi_ids[i],
            )
    ...
    def sinar_city_and_site(self, row, linker):
        ...
        for i, landfill in enumerate(landfills):
            new_landfill = Landfill(
                ...
                mcf=pd.Series(
                    mcfs[landfill], index=range(lifespans[landfill][0], 2074)
                ),
                city_params_dict=city_params_dict,
                city_instance_attrs=baseline.city_instance_attrs,
                landfill_index=i,
                ...
                gas_capture_efficiency=pd.Series(
                    ...
                    index=range(lifespans[landfill][0], 2074),
                ),
                # flaring=pd.Series(flaring, index=year_range),
                # leachate_circulate=leachate_circulate[i],
    ...
    def site_only_estimate(self, row=None, pop_data=None):
        ...
        open_date = row['Site Open Year'].fillna(1980).values[0]
        if open_date < 1980:
            open_date = 1980
        close_date = row['Site Close Year'].fillna(2074).values[0]
    ...
    def _divs_from_component_fractions(
        ...
        parameters.non_compostable_not_targeted_total = pd.Series(
            non_compostable_not_targeted_total, np.arange(1960, 2074)
        )
        if parameters.non_compostable_not_targeted_total.isna().all():
            parameters.non_compostable_not_targeted_total = pd.Series(
                0, index=np.arange(1960, 2074)
            )
        ...
        if not scenario_parameters.sites_method:
            ...
            if new_gas_pct > 0:
                ...
                new_landfill = Landfill(
                    ...
                    close_date=2074,
                    site_type="Sanitary Landfill",
                    mcf=pd.Series(1, index=range(implement_year, 2074)),
                    ...
                    new_baseline=False,
                    gas_capture_efficiency=pd.Series(
                        0.6, index=range(implement_year, 2074)
                    ),
                    # flaring=pd.Series(flaring, index=year_range),
                    # leachate_circulate=leachate_circulate[i],
                    fraction_of_waste_vector=fraction_of_waste_vector,
                    ...
                    oxidation_factor=pd.Series(0.22, index=range(implement_year, 2074)),
                    rmi_id=999999999,
                )
