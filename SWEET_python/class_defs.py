...
class DivsDF(BaseModel):
    ...
    def create(
        ...
        anaerobic = cls._create_dataframe(
            divs.anaerobic,
            start_year,
            end_year,
            year_of_data_pop,
        ...
        combustion = cls._create_dataframe(
            divs.combustion,
            start_year,
            end_year,
            year_of_data_pop,
        ...
        recycling = cls._create_dataframe(
            divs.recycling,
            start_year,
            end_year,
            year_of_data_pop,
    ...
    def _create_dataframe(
        ...
        for year in years:
            t = year - year_of_data_pop
            growth_rate = (
                growth_rate_historic if year < year_of_data_pop else growth_rate_future
            )
            for waste in waste_types:
    ...
    def sum(self) -> pd.DataFrame:
        all_columns = (
            self.compost.columns.union(self.anaerobic.columns)
            .union(self.combustion.columns)
    ...
    def create_simple(
        ...
        def create_div_df(baseline: WasteMasses, scenario: WasteMasses) -> pd.DataFrame:
            ...
            values[baseline_mask, :] = (
                baseline_arr * growth_factors[baseline_mask, None]
            )
            # Assign scenario values where baseline_mask is False
            values[~baseline_mask, :] = (
                scenario_arr * growth_factors[~baseline_mask, None]
            )
        ...
        combustion_df = create_div_df(
            baseline_divs.combustion, scenario_divs.combustion
        )
    ...
    def implement_advanced(
        ...
        compost = cls._apply_growth_rate_advanced(
            divs.compost,
            year_of_data_pop,
            growth_rate_historic,
            growth_rate_future,
        ...
        anaerobic = cls._apply_growth_rate_advanced(
            divs.anaerobic,
            year_of_data_pop,
            growth_rate_historic,
            growth_rate_future,
        ...
        combustion = cls._apply_growth_rate_advanced(
            divs.combustion,
            year_of_data_pop,
            growth_rate_historic,
            growth_rate_future,
        ...
        recycling = cls._apply_growth_rate_advanced(
            divs.recycling,
            year_of_data_pop,
            growth_rate_historic,
            growth_rate_future,
    ...
    @staticmethod
    def _apply_growth_rate_advanced(
        df: pd.DataFrame,
        year_of_data_pop: int,
        growth_rate_historic: float,
        growth_rate_future: float,
...
class LandfillWasteMassDF(BaseModel):
    ...
    def create(
        cls,
        waste_generated_df: pd.DataFrame,
        divs_df: DivsDF,
        fraction_of_waste: float,
    ...
    def create_advanced(
        ...
        waste_generated_df: pd.DataFrame,
        divs_df: DivsDF,
        fraction_of_waste_series: pd.Series,
    ) -> "LandfillWasteMassDF":
        try:
            diverted = divs_df.sum()
        except:
        ...
        landfill_waste = (waste_generated_df - diverted).mul(
            fraction_of_waste_series, axis=0
        )

        return cls(df=landfill_waste)

    # This needs an underscore before it i think
    def dst_implement_advanced(
        self,
        waste_generated_df: pd.DataFrame,
        divs_df: "DivsDF",
        fraction_of_waste_series: pd.Series,
