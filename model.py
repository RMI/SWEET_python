...
class SWEET:
    ...
    def estimate_emissions(self): 
        ...
        for year in range(self.landfill.open_date, self.landfill.close_date):
            ...
            t = year - self.landfill.open_date
        ...
        # How do I deel with compost now...it's separate, but requires it's own loop? Ya I think, just do the time loop again. 
        self.q_df = pd.DataFrame(self.qs)
        self.q_df['total'] = self.q_df.sum(axis=1)
        self.m_df = pd.DataFrame(self.ms)
        
        return self.m_df, self.q_df
